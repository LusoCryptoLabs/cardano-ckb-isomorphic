...
// ---- instrumentation: count Poseidon2 permutations (added for the CKB-VM cost keystone) ----
use core::sync::atomic::{AtomicU64, Ordering};
pub static POSEIDON2_PERM_COUNT: AtomicU64 = AtomicU64::new(0);
/// Reset the global Poseidon2 permutation counter (call right before the region you want to measure).
pub fn poseidon2_count_reset() { POSEIDON2_PERM_COUNT.store(0, Ordering::Relaxed); }
/// Read the global Poseidon2 permutation counter.
pub fn poseidon2_count() -> u64 { POSEIDON2_PERM_COUNT.load(Ordering::Relaxed) }
    ...
    Diffusion: DiffusionPermutation<AF, WIDTH>,
{
    fn permute_mut(&self, state: &mut [AF; WIDTH]) {
        POSEIDON2_PERM_COUNT.fetch_add(1, Ordering::Relaxed);
