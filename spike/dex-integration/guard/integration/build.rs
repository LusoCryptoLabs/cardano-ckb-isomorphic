...
// `../ckb_script/target/riscv64imac-unknown-none-elf/release/leap_mint_guard`; on a clean checkout that file
// does not exist, so every test panics in 0.00s at `fs::read(GUARD_BIN).expect("build guard first")` before
// any CKB-VM logic runs (a false failure that looks like a defect). This script cross-compiles the contract
// first so the suite is green from a clean tree.
use std::path::Path;
use std::process::Command;

const CONTRACT_DIR: &str = "../ckb_script";
const TARGET: &str = "riscv64imac-unknown-none-elf";
const BIN: &str = "leap_mint_guard";
