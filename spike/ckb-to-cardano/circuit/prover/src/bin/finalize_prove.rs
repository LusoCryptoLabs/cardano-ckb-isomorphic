//! Finalize prove stage: the circuit counterpart for the `Finalize` action of cardano_bound.ak (closes
//! re-audit finding N-2). It proves the SAME chain-of-custody as the leap (R1 PoW + R2 ChainRootMMR
//! membership + R3 tx-inclusion + R4 seal binding) on LIVE Pudge state, but instead of a state-transition
//! commitment it computes the seal-specific FINALIZE commitment IN-CIRCUIT:
//!     commitment = blake2b256("FIN" || seal)          (standard blake2b-256, == builtin.blake2b_256)
//! matching cardano_bound.ak `finalize_commitment(seal) = blake2b_256(#"46494e" ++ seal)`. Public inputs (3)
//! = (chain_root, seal, finalize_commitment) - the same shape `inputs_bind` checks for the Finalize branch.
...
#[derive(Clone)]
struct FinalizeLeap { raw:[u8;192], nonce:[u8;16], cbmt_leaf:[u8;32], cbmt_path:Vec<([u8;32],bool)>, wit:[u8;32],
...
impl ConstraintSynthesizer<Fr> for FinalizeLeap {
    fn generate_constraints(self, cs: ConstraintSystemRef<Fr>) -> Result<(), SynthesisError> {
        ...
        // R4 FINALIZE commitment + public inputs (chain_root, seal, finalize_commitment)
        let seal=w(&self.seal,&cs)?;
        // SEC D2: the seal must equal the block-included tx (R3 CBMT leaf) - not a free witness.
        for i in 0..32 { seal[i].enforce_equal(&leaf[i])?; }
        // commitment = blake2b256("FIN" || seal), computed IN-CIRCUIT (the SNARK itself attests the FIN tag).
        ...
        let comm_bytes={let mut c=b"FIN".to_vec(); c.extend_from_slice(&self.seal); b2b(&c)};
...
fn main(){
    ...
    eprintln!("finalize: target block {} hash {} recomputed OK", hs[3]["number"].as_str().unwrap(), &hexs(&bh)[..16]);
    ...
    // SEC D2: the seal IS the block-included tx (= the CBMT leaf).
    let seal=cbmt_leaf;
    // SEC D1/B6: finalize commitment = standard blake2b-256("FIN" || seal) - matches finalize_commitment().
    let commitment={let mut c=b"FIN".to_vec(); c.extend_from_slice(&seal); b2b(&c)};
    let circ=FinalizeLeap{raw,nonce,cbmt_leaf,cbmt_path,wit,leaf0:leaves[3],mpath,chain_root,seal};
    let cs=ConstraintSystem::<Fr>::new_ref();
    {let c2=FinalizeLeap{raw,nonce,cbmt_leaf,cbmt_path:circ.cbmt_path.clone(),wit,leaf0:leaves[3],mpath:circ.mpath.clone(),chain_root,seal};
     c2.generate_constraints(cs.clone()).unwrap(); eprintln!("finalize: circuit is_satisfied={} constraints={}", cs.is_satisfied().unwrap(), cs.num_constraints()); assert!(cs.is_satisfied().unwrap()); }
    let mut rng=ark_std::rand::rngs::StdRng::seed_from_u64(7);
    eprintln!("finalize: Groth16 setup+prove on LIVE data...");
    let setup=FinalizeLeap{raw,nonce,cbmt_leaf,cbmt_path:circ.cbmt_path.clone(),wit,leaf0:leaves[3],mpath:circ.mpath.clone(),chain_root,seal};
    let (pk,vk) = if let Ok(dir)=std::env::var("CEREMONY_OUT") {
        let (pk,transcript)=ckb_consensus_circuit::setup_mpc::run_ceremony(setup, 3, 3, "finalize");
        ckb_consensus_circuit::setup_mpc::save_pk(&pk, &format!("{dir}/finalize_pk.bin"));
        let _ = std::fs::write(format!("{dir}/finalize_transcript.json"), serde_json::to_string_pretty(&transcript).unwrap());
        ...
        Groth16::<Bls12_381>::circuit_specific_setup(setup,&mut rng).unwrap()
    };
    ...
    eprintln!("finalize: arkworks verify = true (LIVE FINALIZE proof for block {})", hs[3]["number"].as_str().unwrap());
    let ic:Vec<String>=vk.gamma_abc_g1.iter().map(g1c).collect();
    let redeemer=serde_json::json!({
      "note":"broadcast-ready Cardano redeemer for cardano_bound.ak FINALIZE spend (CKB->Cardano leap close)",
      "target_block": hs[3]["number"], "chain_root": format!("0x{}",hexs(&chain_root)),
      "seal": format!("0x{}",hexs(&seal)), "finalize_commitment": format!("0x{}",hexs(&commitment)),
