"""unbind_seal.py - LIVE leap-out (preview): spend the seal at binding_lock with Unbind, releasing
the seal NFT to a plain address (no continuing seal at the lock). Exercises binding_lock Unbind."""
...
''' % (pol_cbor, name_cbor)
    subprocess.run([*cardano_net.AIKEN_DOCKER, sh], check=True, capture_output=True, env={**os.environ,"MSYS_NO_PATHCONV":"1"})
    ...
    seal=next(u for u in ctx.utxos(LADDR) if u.output.amount.multi_asset and pc.ScriptHash(bytes.fromhex(SEALPOL)) in u.output.amount.multi_asset.data)
    print("seal UTxO:", str(seal.input.transaction_id)[:16],"#",seal.input.index)
    unbind=pc.Redeemer(pc.RawPlutusData(cbor2.CBORTag(122, [])))   # BindingRedeemer::Unbind (2nd variant)
    ...
    b.add_output(pc.TransactionOutput(addr, pc.Value(2_000_000, nft)))   # seal NFT released to a PLAIN owner addr (not the lock)
    b.required_signers=[vk.hash()]; b.collaterals=[collat]
    tx=b.build_and_sign([sk,csk], change_address=addr)
    txid=cardano_net.submit(tx,ctx)
    inst["unbind_tx"]=txid; json.dump(inst, open(os.path.join(ROOT,"deployed","cardano","preview","seal-instance.json"),"w"), indent=2)
